...
def test_read_starlight(file_path):
    ...
    assert isinstance(summary.header.Clip-Bug, float)
    assert isinstance(summary.header.RC-Crash, float)
    assert isinstance(summary.header.Burn-Inwarning-flags, float)
